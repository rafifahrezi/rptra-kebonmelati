...
/**
 * StatsCard Component: Displays a single statistic card.
 */
...
/**
 * HeroSection Component: Displays a carousel of hero slides.
 */
...
/**
 * RptraStatusSection Component: Displays RPTRA operational status.
 */
...
/**
 * AboutSection Component: Displays information about RPTRA.
 */
...
/**
 * FeaturesSection Component: Displays feature cards.
 */
const FeaturesSection = memo(({ features }: { features: Feature[] }) => (
...
/**
 * EventsSection Component: Displays the latest events.
 */
...
/**
 * LocationSection Component: Displays RPTRA location and map.
 */
...
export default function Home() {
  ...
  // Mock data for hero slides and features
  ...
  /**
   * Fetches RPTRA operational status from the API.
   */
  const fetchStatus = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          // Handle unauthorized access (no token)
          setStatusError("Akses tidak diizinkan. Silakan login sebagai admin.");
          setRptraStatus({ status: false, updatedAt: null, updatedBy: "Sistem" });
          return;
        }
        throw new Error(errorData.message || "Gagal mengambil data operasional");
      }

      const data: RptraStatus = await res.json();
      setRptraStatus(data);
      setStatusError(null);
    } catch (error) {
      console.error("Error fetching operational status:", error);
      setStatusError(error instanceof Error ? error.message : "Terjadi kesalahan server");
      setRptraStatus({ status: false, updatedAt: null, updatedBy: "Sistem" });
    }
  }, []);
   ...
   */
  const formatDateOnly = useCallback((dateString: string | null) => {
  ...
  /**
   * Fetches visit statistics from the API.
   */
  ...
  return (
    <div className="relative">
      <div
        ...
        style={{
          backgroundImage: "url(/logo_rptra_bw_edit.png)",
      ...
      <div className="min-h-screen">
        ...
        {statusError && (
          <div className="container mx-auto px-4 py-4 text-center text-red-600">
            {statusError}
          </div>
        )}
        <AboutSection />
